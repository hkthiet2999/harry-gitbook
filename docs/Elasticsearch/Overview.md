...
`Elasticsearch` - ES: là một `search engine` và là một `Distributed Architecture`, có khả năng mở rộng `Designed for Scaling`- lắp thêm node cho nó là nó auto horizontal scalability. Elasticsearch được kế thừa từ `Lucene Apache`, là một open source được phát triển bằng Java. 
...
Elasticsearch là một search engine, do đó tính năng chính của nó là search:

- Tìm kiếm, quản lý log trong ứng dụng
- Tìm kiếm trong các ứng dụng tìm kiếm
- Tìm kiếm content, data trong trang web
- Tìm kiếm sản phẩm
- Tìm kiếm địa điểm
...
- `Near Real Time (NRT) search`: Elasticsearch trả về giá trị rất nhanh, gần như realtime vì chỉ cần search 1 term là có thể trả về các giá trị liên quan tới term đó.
...
- `Data Analysis and Management` Có khả năng phân tích dữ liệu, quản lý dữ liệu- `Data Analysis and Management` Có khả năng phân tích dữ liệu, quản lý dữ liệu
